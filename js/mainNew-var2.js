...
function random($this) {
    ...
    for ( var i = 0; i < number; i ++ ) {
        ...
        $this.eq(random).appendTo($this.parent());
    }
    var item0 = $('.line3__item').eq(0).addClass('activeItem').attr('data-music').split('music-')[1];
    $('.like3').attr('data-item', item0);
...
function musicLike(content) {
    ...
    if ($('.line3__item').index($('.activeItem')) + 1 < $('.line3__item').length - 1){
        var indexNext = $('.line3__item').index($('.activeItem')) + 1;
    }
    else {
        var indexNext =  $('.line3__item').length - 1;
    }
    var dataMusic = $('.line3__item').eq(indexNext).attr('data-music').split('music-')[1];
    var like = $this.attr('data-like');
    $('.circle3').removeClass('circle3-active');
    console.log($('.music'+index));
    $('.music'+index).attr('checked',false);
    $('#music'+index+like).attr('checked', true);
    if ( flag == true ){
        $('.line3__item').removeClass('activeItem').fadeOut(0);
        $('.line3__item').eq(indexNext).addClass('activeItem').fadeIn(0);
        $('.like3').attr('data-item', dataMusic);
        var numberChecked = $('.music'+dataMusic+'[checked]').attr('value');
        $('.like3__item[data-like="'+numberChecked+'"]').find('.circle3').addClass('circle3-active');
...
function musicLeft(){
    ...
    var LikeNewIndex = $all.eq(indexNew).attr('data-music').split('music-')[1];
    $('.progress3__numbers').text(indexNew+1+'/17');
    var progress = ($('.progress3').width()/17)*(indexActive);
    $('.progress3__scale').animate({'width': progress+'px'}, 500);
    var numberChecked = $('.music'+LikeNewIndex+'[checked]').attr('value');
    $('.circle3').removeClass('circle3-active');
    $('.like3__item[data-like="'+numberChecked+'"]').find('.circle3').addClass('circle3-active');
    $('.like3').attr('data-item', LikeNewIndex);
    $('.line3__item').removeClass('activeItem').fadeOut(0);
...
function nextFour(){
    ...
    if (nameGroups.length > 0){
        ...
        for (var f =0; f < nameGroups.length; f++ ){
            ...
            $('<input id="groupPerson'+f+'5" form="formReg" class="groupPerson-radio groupPerson'+f+'" name="groupPersonn'+f+'" type="checkbox" value="5">').appendTo(form);
            $('<input id="groupPerson'+f+'4" form="formReg" class="groupPerson-radio groupPerson'+f+'" name="groupPersonn'+f+'" type="checkbox" value="4">').appendTo(form);
            $('<input id="groupPerson'+f+'3" form="formReg" class="groupPerson-radio groupPerson'+f+'" name="groupPersonn'+f+'" type="checkbox" value="3">').appendTo(form);
            $('<input id="groupPerson'+f+'2" form="formReg" class="groupPerson-radio groupPerson'+f+'" name="groupPersonn'+f+'" type="checkbox" value="2">').appendTo(form);
            $('<input id="groupPerson'+f+'1" form="formReg" class="groupPerson-radio groupPerson'+f+'" name="groupPersonn'+f+'" type="checkbox" value="1">').appendTo(form);
            $('<input id="groupPerson'+f+'99" form="formReg" class="groupPerson-radio groupPerson'+f+'" name="groupPersonn'+f+'" type="checkbox" value="99">').appendTo(form);
            if ( f == 0 ){
...
function groupLike(content) {
    ...
    $('#groupPerson'+index+like).attr('checked', true);
    if ( flag == true ){
...
function loveMusicChange(content) {
    ...
    $('.lovemusic6').attr('checked', false);
    $('#lovemusic6'+index).attr('checked', true);
...
function checkMusicStyles(){
    ...
    for ( var i=0; i<musicStyles.length; i++) {
        ...
        if (flaginner == false) {
            $('.line3__item[data-music="music-'+i+'"]').addClass('balRequired');
            flag = false;
        }
    }
    if (flag == false ) {
        ...
        var dataNumber = $('.line3__item').eq(0).addClass('activeItem').attr('data-music').split('music-')[1];
        $('.like3').attr('data-item', dataNumber);
        $('.circle3').removeClass('circle3-active');
        var numberChecked = $('.music'+dataNumber+'[checked]').attr('value');
        $('.like3__item[data-like="'+numberChecked+'"]').find('.circle3').addClass('circle3-active');
        /* ---------------------------------- */
        marginLeft = 0;
...
function checkEmail(content) {
    ...
    var birthdayReg = /[0-9]{2}(\.)[0-9]{2}(\.)[0-9]{4}/i;
    var phoneReg = /(\+){1}[0-9]*/i;
    var email = $('.edit-text[name="email"]').attr('data-value');
    var birthday = $('.edit-text[name="date"]').attr('data-value');
    var mobile = $('.edit-text[name="mobile"]').attr('data-value');
    var emailTrue = emailReg.test(email);
    var birtdayTrue = birthdayReg.test(birthday);
    var phoneTrue = phoneReg.test(mobile);
    if ( ( emailTrue == false ) && (content.attr('name') == 'email') ) {
        $('.hoverMail').addClass('hover-none');
        flagMail = false;
    }
    var month = parseInt(birthday.split('.')[1]);
    var year = birthday.split('.')[2];
    var day = parseInt(birthday.split('.')[0]);
    if (( month > 7 ) && (year == 1997 ) && (content.attr('name') == 'date') ) {
        $('.hoverDate').addClass('hover-none');
        flagMail = false;
    }
    if (( month == 7 ) && (year == 1997 ) && (day > 28) && (content.attr('name') == 'date') ) {
        $('.hoverDate').addClass('hover-none');
        flagMail = false;
    }
    if ((year > 1997 ) && (content.attr('name') == 'date') ) {
        $('.hoverDate').addClass('hover-none');
        flagMail = false;
    }
    if ( ( phoneTrue == false ) && (content.attr('name') == 'mobile') ){
        $('.hoverMobile').addClass('hover-none');
        flagMail = false;
    }
    if ( (emailTrue == true ) &&  (birtdayTrue == true ) && (phoneTrue == true ) ) {
        flagMail = true;
    }
}
...
$(function(){
    ...
    var options = {
        ...
        onKeyPress: function(cep, e, field, options){
            console.log(cep[0]);
            if (cep[0] == 3) {
                $('.edit-text[name="date"]').mask('AX.XC.VNZZ', options);
                if (cep[3] == 1) {
                    $('.s').mask('AX.XC.VNZZ', options);
                }
            }

            if (cep[3] == 1) {
                $('.edit-text[name="date"]').mask('AZ.XC.VNZZ', options);
            }
        }
    }
    $('.edit-text[name="date"]').mask('AZ.XZ.VNZZ', options);
    $('.edit-text[name="mobile"]').mask('+7 0000000000');
    /* random */
    random($('.line3__item'));
    random($('.groupList4__item'));
    /* высота экрана */
    heightChange();
    ...
    $('.hover').on('click',  function(){
        ...
        if ( $(this).hasClass('hoverMobile')){
            $('.edit-text[name="mobile"]').focus();
        }
        if ( $(this).hasClass('hoverDate')){
            $('.edit-text[name="date"]').focus();
        }
        if ( $(this).hasClass('hoverMail')){
            $('.edit-text[name="email"]').focus();
        }
    });
    /* ---------------------------------------- */
    ...
    $('.button1').on('click', function(){
        ...
        var   flag = checkRequiredRegistration($(this));

        if (( flag == true ) && (flagMail == true) ){
            console.log(flag);
            startInterview($(this));
        }
